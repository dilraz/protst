...
class ViewProfile extends React.Component {
  ...
  remove() {
    var delete_query = fire
      ...
      .doc(this.state.user.uid)
      .collection('friends')
      .where('userId', '==', this.props.match.params.user)
      .get()
      .then(querySnapshot => {
  ...
  componentDidMount() {
    ...
    firebase.auth().onAuthStateChanged(user => {
      ...
      const snapshot = fire
        ...
        .where('owner_id', '==', this.props.match.params.user)
        .get()
        .then(snapshot => {
          const data = snapshot.docs.map(doc => {
                          ...
                          <tr>
                            ...
                            <td style={{ width: '20%' }}>
                              {this.state.isUser ? (
                                ...
                                <button id="bt" onClick={() => this.add()} className="btn btn-info">
                                  Add Friend
                                </button>
                              )}
                            </td>{' '}
